...
class GUI:
    ...
    def __init__(self, client):
        ...
        self.vid = None # video capture object to hold frames
        self.imageWaitTime = 500 # miliseconds to view image

        #######################################################################
        self.showCapturedImage = True # whether or not to show the webcam image
        #######################################################################
    ...
    def keyPressedEvent(self, event):
        if event.keysym != "Shift_R" and event.keysym != "Shift_L":
            ...
            if self.showCapturedImage:
                ret, frame = self.vid.read()
                
                cv2.imshow('frame',frame) 
                cv2.waitKey(self.imageWaitTime)
